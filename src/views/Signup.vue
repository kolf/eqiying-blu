              ...
              <form @submit.prevent="validateForm" class="column is-6 is-offset-3">
                ...
                <div class="field">
                  <label class="checkbox">
                   ...
                   <a @click="showAgreement=true">用户注册协议</a>
...
<p>1.2 用户在使用本社区的服务之前，应当仔细阅读本协议，并同意遵守本协议及所有社区规则后方可成为本社区会员，一旦用户注册成功，则用户与本社区之间自动形成协议关系，用户应当遵守本协议及所有社区规则的约束。用户在使用特殊的服务或产品时，应当同意接受相关协议后方能使用。未同意本协议内容的用户，无法在本社区发布任何内容，仅能使用浏览功能。</p>

<p>1.3 本协议、社区规则可由本社区随时更新，用户在使用本社区产品时应当及时关注，用户同意本社区不承担通知义务。本社区的通知、公告、声明或其它类似内容是本协议及社区规则的一部分。</p>
...
<p>3.3 如发现用户帐号中含有不雅文字或不恰当名称的，本社区保留取消其用户资格的权利，相应后果由用户自行承担。
   <ul>
...
<p>
  3.4 如发现用户的头像、简介等个人信息中包含违法和不良信息的，本社区保留取消其用户资格的权利。</p>

<p>3.5 用户帐号的所有权归本社区，用户仅享有使用权。未经本社区许可，任何人不得赠与、借用、租用、转让或售卖账号，或者以其他方式许可非初始申请注册人使用账号。</p>
<p>3.6 用户有义务保证密码和帐号的安全，用户利用该密码和帐号所进行的一切活动引起的任何损失或损害，由用户自行承担全部责任，本社区不承担任何责任。如用户发现帐号遭到未授权的使用或发生其他任何安全问题，应立即修改帐号密码并妥善保管，如有必要，请通知本社区。因黑客行为或用户的保管疏忽导致帐号非法使用，本社区不承担任何责任。</p>
...
<p>5.2 在不透露单个用户隐私资料的前提下，本社区有权对整个用户数据库进行分析并对用户数据库进行商业上的利用。</p>
<p>
...
<p>8.3 本协议解释权及修订权归北京一起赢科技有限公司所有</p>
      ...
      <div slot="footer"><a class="button is-primary" @click="showAgreement=false">我已阅读</a></div>
    </modal>
  </section>
</template>
